...
// PR #1057: Unified session telemetry constants
const (
	unifiedSessionDayMs    = 24 * 60 * 60 * 1000
	unifiedSessionWeekMs   = 7 * unifiedSessionDayMs
	unifiedSessionOffsetMs = 3 * unifiedSessionDayMs
)

// getUnifiedSessionID generates a unified session ID based on current time
// This mimics official WhatsApp client behavior to prevent banning
func getUnifiedSessionID() string {
	now := time.Now().UnixMilli()
	id := (now + int64(unifiedSessionOffsetMs)) % int64(unifiedSessionWeekMs)
	return strconv.FormatInt(id, 10)
}

// sendUnifiedSession sends unified_session telemetry to WhatsApp servers
// This is critical for mimicking official client behavior

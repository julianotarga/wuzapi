...
func (cli *Client) decryptMsgSecret(ctx context.Context, msg *events.Message, useCase MsgSecretType, encrypted messageEncryptedSecret, origMsgKey *waCommon.MessageKey) ([]byte, error) {
	...
	// PR #991: Get original sender from key first
	...
	// PR #991: Try decryption with both senders to handle @lid cases
	// The message key may use @s.whatsapp.net while the stored sender uses @lid or vice versa
	for _, origSender := range []types.JID{origSenderFromKey, origSenderStored} {
		...
		plaintext, decryptErr := gcmutil.Decrypt(secretKey, encrypted.GetEncIV(), encrypted.GetEncPayload(), additionalData)
		if decryptErr != nil {
			cli.Log.Warnf("Failed to decrypt secret message with sender %s: %v", origSender.ToNonAD().String(), decryptErr)
			err = decryptErr
			continue
		}
